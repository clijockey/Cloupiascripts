...
function peopleList(token,search,max) {
  //----------------------------------------------------------------------------
  // Author:      Rob Edwards (@clijockey/robedwa@cisco.com)
  // Description: List people in your organization.
  //----------------------------------------------------------------------------
  ...
  //Check if email address or dispay name that have been entered
  isEmail = validateEmail(this.value);
  if (isEmail == true){
    logger.addInfo("Detected an email address therefore will search as required.");
    //body.put("personEmail", this.person);
    this.postURI = '/v1/people/?email='+value;
  } else {
    logger.addInfo("Will search based on a dispaly name as an email wasnt entered");
    //body.put("personId", this.person);
    this.postURI = '/v1/people/?displayName='+value;
  }
  if (max)
    this.postURI = this.postURI+'?max='+max;


  //this.postURI = '/v1/people/'
  logger.addInfo("The URL will be : "+this.postURI);
  // Make Rest call
  ...
  if (statusCode == 400) {
      ...
      this.emails = clean(value, "emails");
      this.displayName = clean(value, "displayName");
      this.avatar = clean(value, "avatar");
...
var value = input.search;
var max = input.max;
var proxyHost = input.proxyHost;
var proxyPort = input.proxyPort;

var result = personDetails(this.token,this.search,this.max);

if(result) {
    logger.addInfo("Successfully obtained details");
    output.personId = result[0];
    output.emails = result[1];
    output.displayName = result[2];
    output.avatar = result[3];
    output.created = result[4];
